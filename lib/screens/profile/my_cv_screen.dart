...
class _MyCvScreenState extends ConsumerState<MyCvScreen>
  ...
  void _showLeaveWithoutPublishingSheet() {
    showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => _LeaveWithoutPublishingSheet(
  ...
  void _showCareerAssistantSheet() {
    showModalBottomSheet<void>(
      ...
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
      builder: (sheetContext) => const _CareerAssistantSheet(),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final cvData = _MyCvData.fromSources(
          ...
          children: [
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  if (!isPublished) ...[
                    const _DraftReviewBanner(),
                    const SizedBox(height: 12),
                  ] else ...[
                    _CvAssistantCard(onAskPressed: _showCareerAssistantSheet),
                    const SizedBox(height: 12),
                  ],
                  _CvHeaderCard(
                    data: cvData,
                  ...
                  _SectionCard(
                    ...
                    child: Column(
                      children: cvData.languages
                          .map((language) => _LanguageRow(language: language))
                          .toList(),
                    ),
                  ),
                  const SizedBox(height: 12),
...
class _DraftReviewBanner extends StatelessWidget {
  const _DraftReviewBanner();
...
class _CvHeaderCard extends StatelessWidget {
  const _CvHeaderCard({
    required this.data,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _ContactRow(icon: 'envelope', value: data.email),
          const SizedBox(height: 8),
          _ContactRow(icon: 'phone', value: data.phone),
          const SizedBox(height: 12),
          const Text(
          ...
          Row(
            children: [
              Expanded(child: _InfoCell(label: 'Gender', value: data.gender)),
              Expanded(child: _InfoCell(label: 'Age', value: data.age)),
            ],
          ),
          const SizedBox(height: 14),
          Row(
            children: [
              Expanded(
                child: _InfoCell(label: 'Citizenship', value: data.citizenship),
              ),
              Expanded(
                child: _InfoCell(label: 'Location', value: data.location),
              ),
            ],
          ),
          const SizedBox(height: 14),
...
class _CvAssistantCard extends StatelessWidget {
  const _CvAssistantCard({required this.onAskPressed});
...
class _CvAssessmentCard extends StatelessWidget {
  const _CvAssessmentCard({
...
class _CareerAssistantSheet extends StatelessWidget {
  const _CareerAssistantSheet();
...
class _LeaveWithoutPublishingSheet extends StatelessWidget {
  const _LeaveWithoutPublishingSheet({
...
class _FloatingActionShelf extends StatelessWidget {
  const _FloatingActionShelf({required this.child});
...
class _AvatarBadge extends StatelessWidget {
  const _AvatarBadge({required this.initials, this.photoPath});
